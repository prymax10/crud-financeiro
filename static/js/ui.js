...
// Funções para renderização, eventos e estado da aplicação
...
const UI = {
    ...
    renderizarGraficoCategorias: function(dados) {
        ...
        // Gradientes para visual mais bonito
        const gradients = colors.map((color, index) => {
            const gradient = ctx.createLinearGradient(0, 0, 0, 400);
            gradient.addColorStop(0, color);
            gradient.addColorStop(1, this.adjustColor(color, -30)); // mais escuro
            return gradient;
        });
